...
usage() {
  cat <<'EOF'
Four-way comparison on MiMo-7B with repeated 000 head only:
  1. Native probabilistic MTP (baseline)
  2. Cactus + MTP
  3. SpecCascade TokenV3 + MTP
  4. Dynamic tree with target-dominant relaxation

Uses 50 samples from GSM8K and 50 from HumanEval with the same protocol.

Usage:
  RUN_TAG=mimo_four_way_50_$(date +%Y%m%d) \
    ./scripts/run_mimo_four_way_comparison_50.sh
EOF
}
...
MTP_TOKENS=3
MIMO_MTP_LAYER_MODE=layer0  # Only use 000 head
MAX_MODEL_LEN=8192
PORT="${PORT:-8000}"
...
MTP_MODEL_PATH="${MTP_MODEL_PATH:-$PROJECT_DIR/models/MiMo-7B-Base-MTP3}"
MODEL_NAME="XiaomiMiMo/MiMo-7B-Base"
RUN_TAG="${RUN_TAG:-mimo_four_way_50_$(date +%Y%m%d_%H%M%S)}"
...
# SpecCascade parameters
CASCADE_RULE="${CASCADE_RULE:-token_v3}"
CASCADE_ALPHA="${CASCADE_ALPHA:-0.5}"

# Validate prerequisites
for path in "$MTP_MODEL_PATH/config.json" "$GSM8K_DATA" "$HUMANEVAL_DATA"; do
  if [[ ! -f "$path" ]]; then
  ...
  case "$method" in
    native_mtp)
      setsid env PORT="$PORT" MODEL_PATH="$MTP_MODEL_PATH" \
        ...
        MTP_TOKENS="$MTP_TOKENS" MIMO_MTP_LAYER_MODE="$MIMO_MTP_LAYER_MODE" \
    ...
    cactus)
      setsid env PORT="$PORT" MODEL_PATH="$MTP_MODEL_PATH" \
        ...
        "$PROJECT_DIR/scripts/serve_mimo_cactus.sh" >"$log_file" 2>&1 &
      ;;
    spec_cascade)
      setsid env PORT="$PORT" MODEL_PATH="$MTP_MODEL_PATH" \
        SERVED_MODEL_NAME="$MODEL_NAME" MAX_MODEL_LEN="$MAX_MODEL_LEN" \
        ...
        "$PROJECT_DIR/scripts/serve_mimo_spec_cascade.sh" >"$log_file" 2>&1 &
      ;;
    dynamic_tree)
      mkdir -p "$tree_dir"
      : > "$audit_file"
      setsid env PORT="$PORT" MODEL_PATH="$MTP_MODEL_PATH" \
        ...
        REMTP_MIMO_PREFILL_ALL_LAYERS=1 \
        REMTP_DYNAMIC_TREE_MTP_ROUTE=repeat_000 \
...
cat <<EOF
MiMo Four-Way Comparison (50 samples, 000 head only)
  samples          : $SAMPLES per dataset
  temperature      : $TEMPERATURE
  max tokens       : GSM8K=$GSM8K_MAX_TOKENS HumanEval=$HUMANEVAL_MAX_TOKENS
  MTP route        : $MIMO_MTP_LAYER_MODE (repeated layer 0)
  methods          : native_mtp, cactus, spec_cascade, dynamic_tree
  output           : $RUN_ROOT
EOF
...
python -m remtp.mimo_four_way_report --run-root "$RUN_ROOT"
echo
